...
timer.Create("espentrefresh", 1, 0, function()
	...
	if espradius != 0 then
		for k, v in pairs(ents.FindInSphere(LocalPlayer():GetPos(), espradius)) do
			...
			elseif string.find(v:GetClass(), "weapon") or string.find(v:GetClass(), "shipment") or string.find(v:GetClass(), "printer") or string.find(v:GetClass(), "money") or string.find(v:GetClass(), "durgz") or string.find(v:GetClass(), "seed") then
		...
		for k, v in pairs(ents.GetAll()) do
			...
			elseif v:IsNPC() then
				table.insert(espnpcs, v)
			elseif string.find(v:GetClass(), "spawned_weapon") or string.find(v:GetClass(), "shipment") or string.find(v:GetClass(), "printer") or string.find(v:GetClass(), "money") or string.find(v:GetClass(), "durgz") or string.find(v:GetClass(), "seed") then
				if not string.find(v:GetClass(), "phys") then
					table.insert(espents, v)
				end
			end
