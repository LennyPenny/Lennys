...
local function survey()
    for k, v in pairs(player.GetAll()) do
        ...
        if IsValid(kp) and string.find(kp:GetClass(), "keypad") and !string.find(kp:GetClass(), "check") and !string.find(kp:GetClass(), "crack") and v:EyePos():Distance(kp:GetPos()) <= 71 then
...
local function drawstuff()
    for k, v in pairs(ents.FindInSphere(LocalPlayer():GetPos(), keypadradius)) do
        if IsValid(v) then
            if string.find(v:GetClass(), "keypad") and !string.find(v:GetClass(), "check") and !string.find(v:GetClass(), "crack") then
                if v != e then
